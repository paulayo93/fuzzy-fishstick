import { Link } from 'react-router-dom'
import { motion } from 'framer-motion'
import { ArrowRight } from 'lucide-react'
import { investorsPage } from '../data/stakeholders'
import { SectionHeading } from '../components/SectionHeading'

export function Investors() {
  const { hero, glance, growth, credit, rfi } = investorsPage

  return (
    <>
      <section className="relative min-h-[52vh] overflow-hidden pt-[4.5rem]">
        <img
          src={hero.image}
          alt=""
          className="absolute inset-0 h-full w-full object-cover"
        />
        <div className="absolute inset-0 bg-navy/80" />
        <div className="relative mx-auto flex min-h-[52vh] max-w-6xl items-end px-5 pb-16 sm:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55 }}
          >
            <p className="text-sm font-semibold tracking-[0.18em] text-copper-soft uppercase">
              {hero.eyebrow}
            </p>
            <h1 className="mt-3 max-w-2xl font-display text-4xl font-extrabold tracking-tight text-white sm:text-5xl">
              {hero.title}
            </h1>
            <p className="mt-4 max-w-xl text-lg text-white/80">
              {hero.description}
            </p>
            <Link
              to="/contact"
              className="mt-8 inline-flex items-center gap-2 bg-copper px-6 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-copper-hover"
            >
              Become an investor
              <ArrowRight size={16} aria-hidden />
            </Link>
          </motion.div>
        </div>
      </section>

      <section className="bg-surface py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-5 sm:px-8">
          <SectionHeading eyebrow="Overview" title={glance.title} />
          <ul className="mt-12 grid gap-8 sm:grid-cols-2">
            {glance.items.map((item, index) => (
              <motion.li
                key={item.title}
                initial={{ opacity: 0, y: 14 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.4, delay: index * 0.06 }}
                className="flex gap-4 bg-white px-6 py-6"
              >
                <img
                  src={item.icon}
                  alt=""
                  className="mt-1 h-10 w-10 shrink-0 object-contain"
                  loading="lazy"
                />
                <div>
                  <h3 className="font-display text-xl font-bold text-navy">
                    {item.title}
                  </h3>
                  <p className="mt-2 text-ink-muted leading-relaxed">
                    {item.description}
                  </p>
                </div>
              </motion.li>
            ))}
          </ul>
        </div>
      </section>

      <section className="relative overflow-hidden">
        <div className="grid min-h-[28rem] lg:grid-cols-2">
          <div className="relative min-h-[18rem]">
            <img
              src={growth.image}
              alt="Partnership and growth"
              className="absolute inset-0 h-full w-full object-cover"
              loading="lazy"
            />
          </div>
          <div className="flex flex-col justify-center bg-white px-5 py-16 sm:px-10 lg:px-14">
            <SectionHeading
              eyebrow="Growth"
              title={growth.title}
              description={growth.body}
            />
          </div>
        </div>
      </section>

      <section className="relative overflow-hidden">
        <div className="grid min-h-[28rem] lg:grid-cols-2">
          <div className="flex flex-col justify-center bg-steel px-5 py-16 sm:px-10 lg:order-1 lg:px-14">
            <SectionHeading
              eyebrow="Strength"
              title={credit.title}
              description={credit.body}
              light
            />
          </div>
          <div className="relative min-h-[18rem] lg:order-2">
            <img
              src={credit.image}
              alt="Credit and financial strength"
              className="absolute inset-0 h-full w-full object-cover"
              loading="lazy"
            />
          </div>
        </div>
      </section>

      <section className="relative overflow-hidden">
        <div className="grid min-h-[28rem] lg:grid-cols-2">
          <div className="relative min-h-[18rem]">
            <img
              src={rfi.image}
              alt="Request investor information"
              className="absolute inset-0 h-full w-full object-cover"
              loading="lazy"
            />
          </div>
          <div className="flex flex-col justify-center bg-navy px-5 py-16 sm:px-10 lg:px-14">
            <SectionHeading
              eyebrow="Engage"
              title={rfi.title}
              description={rfi.body}
              light
            />
            <Link
              to="/contact"
              className="mt-8 inline-flex w-fit items-center gap-2 bg-copper px-6 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-copper-hover"
            >
              Request information
              <ArrowRight size={16} aria-hidden />
            </Link>
          </div>
        </div>
      </section>
    </>
  )
}
